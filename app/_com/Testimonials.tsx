...
import { Reviews } from "../_lib/dataService";

export default async function Testimonials() {
  const data = await Reviews();
  console.log(data);
  return (
    <div className="container mx-auto xl:max-w-[1200px] my-12">
      <h1 className="text-xl uppercase md:text-3xl font-extrabold  ">
        Our Happy customer
      </h1>
      <Marquee>
        <div className="flex items-stretch h-full overflow-hidden py-12 gap-4 ">
          {data?.map((item) => (
            <div
              key={item.id}
              className="border w-[300px] ml-4 rounded-lg px-4 py-2 flex flex-col "
            >
              ...
              <div className="flex items-center gap-1 my-3">
                <h1 className="font-semibold m">{item.UserName}</h1>
                <p>
                  <svg
                    width="24"
                    height="25"
              ...
              <p className="text-gray-500">&ldquo; {item.Review} &ldquo;</p>
            </div>
          ))}
        </div>
      </Marquee>
